...
def insert_node(tree, data, parent, file):
    MAX_RETRY = 10
    cnt = 0
    ...
    while True:
        ...
        try:
            id = data + "_" + str(cnt)
            tag = file + "/" + id
            tree.create_node(tag=tag, identifier=id, parent=parent, data=data)
            return
        except Exception as e:
            cnt += 1
            if cnt >= MAX_RETRY:
        ...
        for leaf in leaves:
            if not is_root:
                if is_implicit(leaf, cg):
                    ...
                    IMPLICIT += 1
                    sink = classof(leaf)
                    tree.update_node(leaf.identifier, data=classof(leaf))
                else:
                    sink = funcof(leaf)
            else:
                sink = funcof(leaf)
            ...
            for caller, callees in cg.items():
                for callee in callees:
                    if callee.endswith(sink):
                        # if callee.endswith('validate.create_function'):
                        #     print(cg)
                        #     print(leaf)
                        #     print(leaf.data)
                        #     print(callee)
                        ...
                        insert_node(tree, whole, leaf.identifier, file)
    ...
    for init_sink in init_sinks:
        ...
        for init_suspect_file in init_matching_files:
            ...
            if "experimental" in init_suspect_file or "example" in init_suspect_file:
                continue
            tree = Tree()
            tree.create_node(tag=init_sink, identifier=init_sink, data=init_sink)
            # generate cg
